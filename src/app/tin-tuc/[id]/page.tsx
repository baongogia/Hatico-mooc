...
const FallbackImage = ({
  ...
  if (!src || error) {
    return (
      <div
        className={cn(
          "w-full h-full flex items-center justify-center text-slate-400 bg-slate-100",
          className,
        )}
      >
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 2. Fallback: if not found and 'id' is a number, try fetching by numeric ID
    ...
    return (
      <div className="w-full min-h-screen pt-32 pb-12 flex flex-col justify-center items-center bg-slate-950 text-white">
        ...
        <p className="text-slate-400 mb-8">
          Có vẻ như bài viết này đã bị gỡ bỏ hoặc đường dẫn không chính xác.
        </p>
        <Link href="/tin-tuc">
          <Button
            variant="accent"
            size="lg"
            className="rounded-sm font-black uppercase tracking-widest text-xs h-12 px-8"
          >
            Quay lại danh sách
          </Button>
        </Link>
      ...
      <section className="relative pt-32 pb-20 overflow-hidden bg-slate-950 border-b border-white/5">
        ...
        <div className="container max-w-5xl mx-auto px-6 relative z-10">
          ...
          >
            <Link
              href="/tin-tuc"
              className="group inline-flex items-center text-xs font-black uppercase tracking-[0.2em] text-slate-300 hover:text-white mb-10 transition-all"
            >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest opacity-80">
                    Tác giả
                  </span>
                  <span className="text-sm font-bold text-white">
                    {article.owner || "Hatico Team"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest opacity-70">
                    Ngày đăng
                  </span>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3].map((i) => (
                    <button
                      key={i}
                      className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-950 hover:text-white transition-all text-slate-400"
                    >
            ...
            <div className="sticky top-32 space-y-12">
              ...
              <div className="p-8 bg-slate-950 relative overflow-hidden group shadow-2xl rounded-sm">
                <div className="relative z-10">
                  <h3 className="text-xl font-black text-white mb-4 leading-none uppercase tracking-tighter">
                    Bản tin công nghiệp
                  </h3>
                  <p className="text-[10px] text-slate-300 mb-8 font-bold uppercase tracking-widest">
                    Đăng ký nhận tin từ Hatico
                  </p>
                  <input
                    type="email"
                  ...
                  <Button
                    variant="accent"
                    className="w-full font-black uppercase text-[10px] tracking-[0.2em] h-14 rounded-none"
                  >
                    Đăng ký ngay
                  </Button>
                </div>
