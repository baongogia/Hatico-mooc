...
export default function NewsPage() {
  ...
  const categories = [
    "Tất cả",
    ...new Set(articles.map((a) => a.type).filter(Boolean) as string[]),
  ];
    ...
    <div className="w-full min-h-screen bg-slate-50 flex flex-col">
      ...
      <section className="relative pt-32 pb-20 bg-slate-950 overflow-hidden">
        ...
        <div className="container max-w-7xl mx-auto px-6 relative z-10">
          <motion.div
            ...
            className="max-w-3xl"
          >
            <span className="text-slate-400 font-black uppercase tracking-[0.4em] text-[9px] mb-4 block opacity-80">
              Hatico Media Center
            </span>
            ...
            <p className="text-lg text-slate-300 font-medium leading-relaxed max-w-xl">
              Cập nhật những chuyển động mới nhất của ngành vận tải, công nghệ
              rơ moóc và các cột mốc quan trọng của Hatico trên hành trình nâng
              tầm logistics Việt.
            </p>
          </motion.div>
        </div>
      ...
      <section className="sticky top-16 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 py-4">
        <div className="container max-w-7xl mx-auto px-6 flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar relative">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setActiveCategory(cat)}
                className={cn(
                  "px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all relative",
                  activeCategory === cat
                    ? "text-white"
        ...
        {loading ? (
          ...
          <div className="text-center py-32">
            ...
            <p className="text-slate-500">
              Thử chọn một danh mục khác hoặc tìm kiếm từ khóa khác.
            </p>
          </div>
        ) : (
          <motion.div
        ...
        <div className="container max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-black text-white mb-6 uppercase tracking-tighter">
              Đăng ký bản tin công nghiệp
            </h2>
            <p className="text-slate-300 mb-10 max-w-xl mx-auto font-medium">
              Đừng bỏ lỡ các thông tin quan trọng về kỹ thuật và biến động thị
              trường vận tải nặng.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              ...
              <Button
                variant="accent"
                className="font-black uppercase tracking-widest text-[10px] px-8 h-14"
              >
                Tham gia
              </Button>
            </div>
